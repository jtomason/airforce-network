...
var updateChannel = require('./app/update-channel.js');
...
app.post('/channels/:channelName/peers', async function(req, res) {
	...
	let message =  await join.joinChannel(channelName, peers, req.username, req.orgname).catch((error)=> res.send(error.toString()));
	
	res.send(message);
});

// Update Channel for anchor peers
app.post('/channels/:channelName/update', async function(req, res) {
	logger.info('<<<<<<<<<<<<<<<<< UPDATE  C H A N N E L >>>>>>>>>>>>>>>>>');
	var channelName = req.params.channelName;
	var anchorPeerTxFile = req.body.anchorPeerTxFile;
	

	let message =  await updateChannel.updateChannel(req.orgname,channelName, anchorPeerTxFile, req.username);
	res.send(message);
});
